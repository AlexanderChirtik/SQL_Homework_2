...
ALTER TABLE posts ADD client_id INT;
ALTER TABLE posts ADD FOREIGN KEY (client_id) REFERENCES clients(id);
UPDATE posts SET client_id = 1 WHERE id = 1;
UPDATE posts SET client_id = 3 WHERE id = 2;
UPDATE posts SET client_id = 5 WHERE id = 3;
UPDATE posts SET client_id = 3 WHERE id = 4;
UPDATE posts SET client_id = 4 WHERE id = 5;
UPDATE posts SET client_id = 1 WHERE id = 6;
UPDATE posts SET client_id = 2 WHERE id = 7;
UPDATE posts SET client_id = 2 WHERE id = 8;
UPDATE posts SET client_id = 4 WHERE id = 9;
SELECT id, title, full_text, user_id, client_id FROM posts;
...
SELECT p.title, p.full_text, p.client_id, c.login FROM posts AS p, clients AS c WHERE p.client_id = c.id;

-- №3.	Выполните поиск  по публикациям, автором котоырх является клиент "Mikle".
SELECT p.id, p.title, p.full_text, p.user_id, p.client_id, c.login FROM posts AS p, clients AS c WHERE p.client_id = c.id AND c.login = "Mikle";
